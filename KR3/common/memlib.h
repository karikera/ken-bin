...
namespace kr
{
	...
	#define _VOID_				ATTR_NOALIAS static void CT_FASTCALL
	#define _RETURN_(type)		ATTR_CHECK_RETURN ATTR_NOALIAS static type CT_FASTCALL
	...
	struct memt
	{
		...
		_VOID_ xor_copy(ptr _dest, cptr _src, size_t len, dword key) noexcept;
		...
		template <typename T>
		_NOTNULL_(T*) find(T* _src, std::add_const_t<T>* _tar, size_t _tarlen) noexcept;
		template <typename T>
		_NOTNULL_(T*) find_e(T* _src, atype _tar, size_t _srclen) noexcept;
		template <typename T>
		_NULLABLE_(T*) find(T* _src, std::add_const_t<T>* _tar, size_t _srclen, size_t _tarlen) noexcept;
		template <typename T>
		_NULLABLE_(T*) find_r(T* _src, atype _tar, size_t _len) noexcept;
		template <typename T>
		_NULLABLE_(T*) find_r(T* _src, std::add_const_t<T>* _tar, size_t _srclen, size_t _tarlen) noexcept;
		template <typename T>
		_NULLABLE_(T*) find_ry(T* _src, std::add_const_t<T>* _tar, size_t _srclen, size_t _tarlen) noexcept;
		template <typename T>
		...
		template <typename T>
		_NOTNULL_(T*) find_ye(T* _src, std::add_const_t<T>* _tar, size_t _srclen, size_t _tarlen) noexcept;
		template <typename T>
		...
		template <typename T>
		_NOTNULL_(T*) find_ny(T* _src, std::add_const_t<T>* _skp,size_t _skplen) noexcept;
		template <typename T>
		_NULLABLE_(T*) find_ny(T* _src, std::add_const_t<T>* _skp, size_t _srclen, size_t _skplen) noexcept;
		template <typename T>
		_NULLABLE_(T*) find_nry(T* _src, std::add_const_t<T>* _skp, size_t _srclen, size_t _skplen) noexcept;
		template <typename T>
		_NULLABLE_(T*) find_nye(T* _src, std::add_const_t<T>* _skp, size_t _srclen, size_t _skplen) noexcept;
		template <typename T>
		_RETURN_(size_t) pos(T* _src, atype _tar) noexcept;
		template <typename T>
		_RETURN_(size_t) pos(T* _src, atype _tar, size_t _srclen) noexcept;
		template <typename T>
		_RETURN_(size_t) pos(T* _src, std::add_const_t<T>* _tar, size_t _tarlen) noexcept;
		template <typename T>
		_RETURN_(size_t) pos(T* _src, std::add_const_t<T>* _tar, size_t _srclen, size_t _tarlen) noexcept;
		template <typename T>
		_RETURN_(size_t) pos_r(T* _src, atype _tar, size_t _srclen) noexcept;
		template <typename T>
		_RETURN_(size_t) pos_ry(T* _src, std::add_const_t<T>* _tar, size_t _srclen, size_t _tarlen) noexcept;
		template <typename T>
		_RETURN_(size_t) pos_y(T* _src, std::add_const_t<T>* _tar, size_t _tarlen) noexcept;
		template <typename T>
		_RETURN_(size_t) pos_y(T* _src, std::add_const_t<T>* _tar, size_t _srclen, size_t _tarlen) noexcept;
		template <typename T>
		_RETURN_(size_t) pos_nry(T* _src, std::add_const_t<T>* _tar, size_t _srclen, size_t _tarlen) noexcept;
		...
		_RETURN_(bool) isdbcs(cptr _src, size_t _pos) noexcept;

		template <typename T>
		_RETURN_(T) toint(cptr _src, size_t _len, uint _radix) noexcept;
		template <typename T>
		_RETURN_(T) toint_limit(cptr _src, size_t _len, uint _radix); // OutOfRangeException
		template <typename T>
		_RETURN_(T) tosint(cptr _src, size_t _len, uint _radix) noexcept;
		template <typename T>
		_RETURN_(T) tosint_limit(cptr _src, size_t _len, uint _radix); // OutOfRangeException
		template <typename T>
		_RETURN_(T) touint(cptr _src, size_t _len, uint _radix) noexcept;
		template <typename T>
		_RETURN_(T) touint_limit(cptr _src, size_t _len, uint _radix); // OutOfRangeException
		template <typename T>
	...
	namespace _pri_
	{
		...
		template <bool OP> struct ARRCOPY;
		template <> struct ARRCOPY<true>
		{
	...
	#pragma warning(disable:6326)
		template <typename T> _NOTNULL_(T*) find(const T* _src, const T &_tar) noexcept;
		template <typename T> _NULLABLE_(T*) find(const T* _src, const T &_tar, size_t _len) noexcept;
		template <typename T> _RETURN_(dword) pos(const T* _src, const T &_tar) noexcept;
		template <typename T> _RETURN_(dword) find(const T* _src, const T &_tar, size_t _len) noexcept;
