...
namespace kr
{
	...
	public:
		...
		CastIterable(Iterable * iterable) noexcept
			:m_iterable(iterable)
		{
		}

		CastIterator<Iterator, T> begin() noexcept
		{
			return m_iterable->begin();
		}
		CastIterator<Iterator, T> end() noexcept
		{
	...
	namespace _pri_
	{
		...
		{
			static_assert(is_base_of_v<NodeLinkData, Node>, "Node must inherit NodeLinkData");
			using Super = Base;
		protected:
			using Super::m_axis;
